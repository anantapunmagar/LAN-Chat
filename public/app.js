...
function addMessage(m) {
  ...
  if (key === state.active) {
    ...
    else box.append(...messageBits(m, arr[arr.length - 2]));
...
async function poll() {
  if (!state.me) return;
  try {
    const d = await api(`/api/poll?since=${state.pollSince}`);
    state.pollSince = d.ts;
    ...
    // typing
    for (const t of d.typing) {
      let map = state.typing.get(t.conv);
      if (!map) { map = new Map(); state.typing.set(t.conv, map); }
      map.set(t.from, Date.now() + 4000);
    }
    renderTyping();
  } catch (e) {
...
function sendTyping() {
  const now = Date.now();
  if (now - state.lastTypingSent < 1800) return;
  state.lastTypingSent = now;
  api('/api/typing', { body: { conv: state.active } }).catch(() => {});
}
